...
:@param plan: (str) Directory containing formatted sample data sheets
...
# paths['plan'] = "/oasis/tscc/scratch/mioverto/plan/SK_SG_planfile.txt"
# paths['data'] = "/oasis/tscc/scratch/mioverto/data/MAseq2/reads"
# paths['fastq'] = "/oasis/tscc/scratch/mioverto/data/MAseq2/reads"
...
with open( paths['plan'], 'r') as sample_table:
    ...
    for sample_row in sample_table:
        ...
        # assert len(sample_row.split("\t")) == 5, "Required headers in planfile: batch, sample, Tx, lineage, ID")
        #Separating individual items in sample data
        batch, sample, Tx, lineage, ID = sample_row.split()
        ...
        for file_name in sources:
            ...
            #Matching to the unique portion of the file name + R2
                ...
                pass
        #Generate destination file names from elements in plan file; replace ## with strain
        dest1 = '{0}/{1}_R1.fastq'.format(paths['fastq'], ID)
        dest2 = '{0}/{1}_R2.fastq'.format(paths['fastq'], ID)
        os.rename( src1, dest1 )
        os.rename( src2, dest2 )
        #Reset the paths for error detection
        src1, src2, dest1, dest2 = ['', '', '', '']

# for filename in "1x1"*; do echo mv \"$filename\" \"${filename//1x1/full}\"; done | /bin/bash
